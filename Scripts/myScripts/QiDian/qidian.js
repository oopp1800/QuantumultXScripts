...
⏰定时任务:
[task local]
30 10 * * * https://raw.githubusercontent.com/oopp1800/QuantumultXScripts/main/Scripts/myScripts/QiDian/qidian.js, img-url=https://raw.githubusercontent.com/chxm1023/Script_X/main/icon/qidian.png, tag=起点读书, enabled=true
📦BoxJs地址:
https://raw.githubusercontent.com/oopp1800/QuantumultXScripts/main/mcdasheng.boxjs.json
@params: 
    "qd_tasks"
    "qd_tasks_last_date"
...
(async () => {
  for (const task of tasks) {
    for (let i = 0; i < task.taskRemainTimes; i++) {
      $.info("当前执行任务：", JSON.stringify(task));
      if (!task.session) {
        continue;
      }
      $.info(`🟡${task.taskType}任务执行第${i + 1}次`);
      await runTask(task.session, () => { $.setdata("qd_tasks_last_date", ""); });
...
async function runTask(session, errorCallback) {
  return $.http.post(session).then((resp) => {
...
/**
 * Env.js
 * https://github.com/chavyleung/scripts?tab=readme-ov-file#%EF%B8%8Fenvjs 
 */
